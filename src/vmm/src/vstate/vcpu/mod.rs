...
impl PartialEq for VcpuEmulation {
...
mod tests {
    ...
    impl Vcpu {
        pub fn emulate(&self) -> std::result::Result<VcpuExit, errno::Error> {
            if (&*self.vcpu_exit_reason.lock().unwrap()).is_some() {
                return self.vcpu_exit_reason.lock().unwrap().take().unwrap();
            }
            Err(errno::Error::new(libc::SIGILL))
        }
    }
    ...
    fn test_run_emulation() {
        ...
        *(vcpu.vcpu_exit_reason.lock().unwrap()) = Some(Err(errno::Error::new(libc::ENOSYS)));
        ...
        assert_eq!(
            ...
            format!(
                "{:?}",
                EmulationError::FaultyKvmExit("Function not implemented (os error 38)".to_string())
            )
        );
